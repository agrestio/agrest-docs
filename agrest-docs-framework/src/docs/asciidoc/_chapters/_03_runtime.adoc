...
that ensures they are automatically merged into the runtime if present on the classpath.

NOTE: You may not be using this often in your own code, as it is much easier to create inline modules when assembling
the runtime. But this is a useful mechanism for creating Agrest extensions.

To turn your code into an
