...
Here is how you may program a GET request handler:
...
<2> Pass all request parameters from JAX-RS `UriInfo` object
The framework will process those that are defined in the Agrest protocol (`exp`, `include`, etc.) and ignore the rest
<3> Execute the select operation

=== GET a Single Object

In the example above we returned a collection of objects in the form of `DataResponse`. Often REST API would only
return a single object for a given `id`. Here is how to implement this in Agrest:
...
Another  common pattern in the REST API design is retrieving "children" of a given "parent" object. E.g. the books of
a single known author:
...
NOTE: This is pure syntactic sugar for "pretty" URLs like `/author/576/books`. Similar functionality can be implemented
as `/book?exp=author=576` or `/author/576?include=books`.
...
=== Server-side Protocol Parameter

The `AgRequest` object has another useful property. Unlike `UriInfo` that captures everything that was passed in the
URL by the client, it is manually constructed *on the server*. So you can emulate parts of the Agrest protocol
in your application code to limit what the client can do or to define the default data appearance, such as ordering:
...
=== Customizing Request Entities

As already mentioned in the chapter on <<Entity Overlays,Entity Overlays>>, you can redefine the shape of the entities,
...
<1> Conditionally define that some attributes are only visible to the app administrators
<2> Attach overlay to the request
