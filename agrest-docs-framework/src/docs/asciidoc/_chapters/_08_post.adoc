...
collection of objects conforming to Agrest Protocol link:../protocol#update-request[Update Document] format. Alternatively,
...
* It allows to tweak the payload of the client request in your server-side code

NOTE: These update data formats are used across all handlers that create or modify entities, and both POST and PUT
requests.
