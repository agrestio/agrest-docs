...
----
Restart the application and go to http://127.0.0.1:8080/swagger-ui.html . You should see the navigable UI for your
API docs similar to the one in this screenshot:

image::img/getting-started/swagger-ui.png[align="center"]

You can make your model even more informative by annotating `BookApi` and `AuthorApi` with Swagger annotation
(such as `@OpenAPIDefinition`, `@Operation`, etc.). We'll leave it as an exercise for the user.
